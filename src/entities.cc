...
namespace entities {
    ...
    const char *get_named_entity(const char *name)
    {
    ...
    std::size_t putc_utf8(unsigned long cp, char *buffer)
    {
    ...
    bool simple_parse_entity(const char* from, char* to){
        ...
        if (from[1] == '#') {
            ...
            std::size_t len = putc_utf8(cp, to);
            to[len] = 0;
            return 1;
        } else {
    ...
    std::size_t decode_html_entities_utf8(char *dest, const char *src)
    {
        if(!src) src = dest;
        ...
        {
            memmove(to, from, (std::size_t)(current - from));
            to += current - from;
        ...
        std::size_t remaining = strlen(from);

        memmove(to, from, remaining);
        to += remaining;
        *to = 0;

        return (std::size_t)(to - dest);
    }
}
